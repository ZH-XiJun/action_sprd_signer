...
>
> 需要上传的文件: `你想要签名的分区.img` `vbmeta.img`
...
2. **把镜像上传到一个能获取文件直链的地方。一个传统的方法是，你可以把你的镜像上传到你的仓库里，然后用"View raw"拿到文件直链**
![image](.res/2.png)
![image](.res/3.png)

- **但是你想想啊我tm都上传到这个仓库里了，这工作流不是可以直接自取了吗，还费那劲拿直链那不傻福吗？所以现在只要命名正确，就可以省下获取直链那一步**
...
- 如果你用的是`Sign image (Legacy)`，你只用填一个参数就行了。（甚至可以全部不填）
...
- 可能有人不会填第四个参数，这里笔者给个办法，就是完整回读一下boot/recovery分区，多余部分不用裁掉，然后把提出来的文件的大小填进去就行了。**如果你操作有误，文件大小可能会发生变化，那样就不准了**
